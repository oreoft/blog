...
5. Test类使用“Test”做为结尾 ``StudyRoomInitTest``
6. 控制器使用“Controller”结尾 ``LearningGroupSubChannelController``
7. 业务处理接口使用“Service”结尾(**不建议**使用I开头)，实现类使用“ServiceImpl”结尾 ``e.g. LearningFeedStoryStatusServiceImpl``
8. 持久层的封装的话...结尾使用“Dao”、"Mapper"、"Repository"都可以，主要还是看项目一以贯之的风格和团队习惯，如果是我主导的新项目我会把mongdb、redis、influxdb等nosql或者多存储介质联合获取命名成Repository结尾，mysql、postgrep之类的RDBMS命名为Dao结尾 ``AggHealthMongoRepository``
