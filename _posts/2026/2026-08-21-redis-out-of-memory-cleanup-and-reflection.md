---
layout: post
title: 一次生产 Redis 内存打满的紧急排查、精准清理与架构复盘
excerpt: 线上 Redis 内存飙到 99.98% 触发逐出报警，看我们如何通过 LRU 空闲时间精准清理 50% 冷数据释放 4GB 内存，并复盘背后的架构隐患
category: middleware
...
- 内存水位直接从 **99.98% 降回 56.5%**，监控大盘上的 `Evictions` 彻底归零，Session 等核心业务警报完全解除；
- 下游 LLM 接口没有出现任何流量冲击，平稳渡过危机。
...
这种设计显然不合理，未来必须把**搜索/算法等高消耗缓存**与**核心业务 Redis** 做物理拆分。
